...
% Opcional: Guardar el archivo codificado
save_option = input('¿Guardar la secuencia binaria en un archivo? (s/n): ', 's');
if save_option == 's'
    output_file = input('Ingrese el nombre del archivo de salida (con extensión .txt): ', 's');
    fid = fopen(output_file, 'w');
    fprintf(fid, '%s', bitstr);
    fclose(fid);
    disp(['Archivo guardado como ', output_file]);
end
